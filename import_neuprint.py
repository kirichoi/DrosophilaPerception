...
criteria_APL = NeuronCriteria(type='^APL.*', regex=True, status='Traced', cropped=False)

PAM_df, PAM_roi_df = fetch_neurons(criteria_PAM)
PPL_df, PPL_roi_df = fetch_neurons(criteria_PPL)
APL_df, APL_roi_df = fetch_neurons(criteria_APL)
...
KCAPLneuron_df, KCAPLconn_df = fetch_adjacencies(KC_df['bodyId'], APL_df['bodyId'])
...
if SAVE:
    ...
    APL_df.to_pickle(r'./data/APL_df.pkl')
    ...
    KCAPLneuron_df.to_pickle(r'./data/neuron_KCAPL_df.pkl')
    KCAPLconn_df.to_pickle(r'./data/conn_KCAPL_df.pkl')
